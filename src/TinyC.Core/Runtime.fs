...
module Runtime =
    type Value = NumberValue of int | TextValue of string
    ...
    type private Slot = Scalar of TcType * int ref | Array of TcType * int array
    ...
    let private number = function NumberValue n -> n | TextValue _ -> raise(RuntimeFailure "Expected a numeric value")
    ...
    let run (options: RunOptions) (program: Program) : Result<RunResult,string> =
        try
            ...
            let allocate (frame: Dictionary<string,Slot>) (d: Declaration) (length: int option) =
                ...
                frame[d.Name] <- match length with Some n -> Array(d.Type, Array.zeroCreate n) | None -> Scalar(d.Type, ref 0)
                ...
                | None ->
                    ...
                    | Some fn ->
                        ...
                        List.zip fn.Parameters args |> List.iter(fun (p,v) ->
                            ...
                            locals[p.Name] <- Scalar(p.Type, ref(number v)))
            ...
            and eval (env: Environment) (expr: Expr) =
                ...
                | Text x -> TextValue x
                | Variable name -> match findSlot env name with Scalar(_, r) -> NumberValue r.Value | Array _ -> raise(RuntimeFailure(sprintf "Array '%s' requires an index" name))
                | Index(name,index) ->
                ...
                | Binary(op,a,b) ->
                    let x,y=eval env a |> number, eval env b |> number
                    let yes v=NumberValue(if v then 1 else 0)
                    match op with
                    | Add -> NumberValue(x+y) | Subtract -> NumberValue(x-y) | Multiply -> NumberValue(x*y)
